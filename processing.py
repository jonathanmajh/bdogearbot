...
def add_gear(gear_type, ctx):
    if len(ctx.message.attachments) == 1:
        ...
        if gear_data.status:
            ...
            filename, file_ext = os.path.splitext(ctx.message.attachments[0].filename)
            ...
            gear_data = update_gear(conn, gear_data)
...
def get_gear(user_id, gear_type=None):
    ...
    if len(results) == 0:
        ...
        for result in results:
            msg = msg + f'{result[7]} {result[1]}: {result[4]}/{result[3]}/{result[5]}: GS: {result[6]}. Updated: {result[9]}\n'
...
def get_average(guild_id, gear_type):
    ...
    conn = create_connection(DB_PATH)
    results = find_average(conn, find) 
